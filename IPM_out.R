# The purpose of this file is to take the bundled data from IPM_dat.R and the models from IPM_mod.R (or IPM_mod1.R - this is a new file with just one model for simplicity.) to make an age-structured, state-space model for capelin.  Model outputs are fed to IPM_out_diag.Rmd which is run through the IPM_master.R file which renders the output, i.e., makes all the figures.

## IPM_JAGS_settings is a helper file to indicate what model and what paramaters JAGS should estimate.

## IPM_exp.R is code for dealing with many of teh problems i've encountered  - see "Problems"
   ### Shaekel approach 
   ### for IPM_mod1.R
   #### Demonstration of beta distribution with simulated and real values.  
   #### This approach seems to give a reasonable s without the Shakell approach.  
   ### "2010" problem
   #### show cohorts by year with model predicted values
   #### Mariano suggestions - does cohort decline exponentially
   #### Do this by mature and immature
   #### # compare I and N
   #### First 2010 problem figure and spin-offs including the process equation (N which is better, i.e., N2t > N3t+1).


## IPM_scratch_pad.R is just a place to work out bits of code


# Problems ----
## Although all models capture the collapse and general trends, the problem with all model iterations to date is that it seems to converge on the mean of a period, e.g. from 1985-1990, it cuts through the values, from ~ 1999- 2005, the model is above the values and then below from 2007-2019.
## I was also having problems getting more complex versions of the model, i.e., those with maturity to converge. The problem here was that I was using the normal distribution to model maturity values and some would invariably be < 0 which would give invalid values when converted to log scale.  This was solved by using the Beta distribution.
## I was also getting really wonky survival values - this also seems to have improved
## For some reason, this iteration produces a variable "si" which plots in IPM_exp.  However, I cannot find any code for it nor is it in the JAGS output.
## Also, I can't seem to get Pearson one-step-ahead resids
## Still don't really get process error
...
# Don't need the next line because not using logits but useful for showing 
