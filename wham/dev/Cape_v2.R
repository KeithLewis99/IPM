...
library(dplyr)
library(tidyr)
Cap_data <- readRDS("capelin/data/CapData.rds")

source("capelin/data/approx_catch_paa_from_figure.r")
...
landings <- read.csv("capelin/data/capelin_landings_kt.csv") |> 
    subset(Year %in% years)

iaa <- read.csv("capelin/data/capelin_abundance_1985-2022.csv") |> 
    right_join(data.frame(year = years), by = "year") |> 
    select(year, I1, I2, I3, I4, I5, I6) |> 
    arrange(year)

baa <- read.csv("capelin/data/capelin_biomass_1985-2022.csv") |> 
    right_join(data.frame(year = years), by = "year") |> 
    select(year, bio1, bio2, bio3, bio4, bio5, bio6) |> 
    arrange(year)

can_spring <- read.csv("capelin/data/can_spring_acoustic_biomass.csv", col.names = c("year", "can_spring", "lwr", "upr"))
larval_den <- data.frame(year = 1985:2024, larval_den = Cap_data$LD)
can_fall <- read.csv("capelin/data/can_fall_acoustic_biomass.csv", col.names = c("year", "can_fall"))
ussr_fall <- read.csv("capelin/data/ussr_fall_acoustic_biomass.csv", col.names = c("year", "ussr_fall"))
ussr_spring <- read.csv("capelin/data/ussr_spring_acoustic_biomass.csv", col.names = c("year", "ussr_spring"))
