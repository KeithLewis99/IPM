...
# Survival/Mortality
   ...
   ##OR -- backward RW
   for(t in 2:11){
      eCO[t-1] ~ dnorm(eCO[t], tau.co)
      #eCO starts in 1984 for Z_1985 -- eCO[1] is 1984, eCO[11] is 1994, & CO[11] is 1995
   }
   eCO[11] ~ dnorm(CO[11], tau.co)
   eCO[12] ~ dnorm(CO[37], tau.co)
   eCO[13] ~ dnorm(eCO[12], tau.co)
   eCO[14] ~ dnorm(eCO[13], tau.co)
   for(a in 1:Ni){
      #last 3 years with no CO
...
# Expected Abundance
   ...
   # Note: Z age indices are offset by 1 -- Z[,1] is the Z for rec, and Z[,2] for N[,1]
