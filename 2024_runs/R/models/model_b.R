...
# Survival/Mortality
   ...
   #Here eCO is offset by 1 year in the past, for eCO[1] is for 1984 which is needed for Z_1985
   eCO[1] ~ dnorm(0, 1/1.44)
   for(a in 1:Ni){
      Z[1,a] <- gamma[a]*4*TI[1]/delta[a]*(1-TI[1]/delta[a]) +
                      epsilon[a]*eCO[1] +
                      ZaA[a]
   }
   for(t in 2:11){
      eCO[t] ~ dnorm(eCO[t-1], tau.co)
      for(a in 1:Ni){
