MyBUGSChains <- function(xx, vars=NULL, PanelNames = NULL){
...
#' MyBUGSHist1
#'
#' @param Output - output of the JAGS model
#' @param SelectedVar - a vector of variables
#' @param PanelNames 
#' @param transform - back transforms the tice values to make them more intepretable
#'
#' @return a graph of histograms with a vline at 0 and the credible
#' @export
#'
#' @examples MyBUGSHist1(out, vars, transform = "yes")
MyBUGSHist1 <- function(Output  = Output, SelectedVar = SelectedVar, PanelNames = NULL, transform = transform){
     #Small function to make an histogram of the ACF per chain.
     #xx$BUGSoutput is the out object from JAGS
     #vars is a character string of variables in xx
     #PanelNames are matching names for the panels
...
MyBUGSOutput <- function(Output  = Output, SelectedVar = SelectedVar, VarNames = NULL){
	...
	x <- xx$sims.matrix
    OUT <- matrix(nrow = length(vars), ncol=5) 
    j<-1
	for(i in vars){
	  xi <- x[,i]	
	...
	colnames(OUT) <- c("mean", "se", "2.5%", "97.5%", "median")
